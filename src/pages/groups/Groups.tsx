    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
        <div>
          ...
          <p className="text-studyrat-secondary text-sm mt-1">Manage and access your study groups</p>
        </div>
        <div className="flex gap-2 items-center">
          <div className="relative">
            <Search size={18} className="absolute left-2.5 top-2.5 text-studyrat-secondary" />
            <Input 
      ...
      {isLoading ? (
          ...
          {filteredGroups.map((group) => (
            <Link key={group.id} to={`/groups/${group.id}`}>
              <Card className="bg-studyrat-border/20 border-studyrat-border hover:border-studyrat-purple transition-colors h-full">
                ...
                <CardContent className="flex items-center gap-2 text-xs text-studyrat-secondary">
                  <Clock size={14} /> 
                ...
                <CardFooter className="border-t border-studyrat-border pt-3 flex justify-between items-center">
                  ...
                  <div className="text-xs">
                    <span className="text-studyrat-purple font-semibold">#{group.inviteCode}</span>
                  </div>
                </CardFooter>
              </Card>
            </Link>
        ...
        <div className="text-center py-12 space-y-4">
          ...
          <Link to="/groups/create">
            <Button className="bg-studyrat-purple hover:bg-studyrat-purpleLight">Create your first group</Button>
          </Link>
        </div>
      )}
